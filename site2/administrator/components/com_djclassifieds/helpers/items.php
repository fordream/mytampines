...
foreach($main_cat as $cat){
	...
	$cat->getSortList($main_id,$lists_const,$id, $all_parent);			
		...
		$name = JHTML::link('index.php?option=com_djclassifieds&task=show&cid='.$cat->id.'&limit='.$limit.'&limitstart='.$limitstart, $cat->name);
		?><li class="level0"><?php
		echo ($name);
		?></li><?php
	}
...
public function getSortList(& $list,& $lists_const,& $main_id,& $all_parent,& $sprawdzone=Array()){
	...
	$li_parent= count($all_parent);
			...
			$this->name = JHTML::link('index.php?option=com_djclassifieds&task=show&cid='.$list->id.'&limit='.$limit.'&limitstart='.$limitstart, $this->name);
...
public function getCategory($id){
		...
		$db= &JFactory::getDBO();
		$query = "SELECT * FROM #__djcat_categories WHERE id =".$id;
		$db->setQuery($query);
...
public function getMainCategories(){
		...
		$db= &JFactory::getDBO();
			$query = "SELECT * FROM #__djcat_categories WHERE parent_id=0";
			$db->setQuery($query);
...
class DjclassifiedsModelCategories{
	...
	function getCategories(){
		...
		$db= &JFactory::getDBO();
			$query = "SELECT * FROM #__djcat_categories";
			$db->setQuery($query);
