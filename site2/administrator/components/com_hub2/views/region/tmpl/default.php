...
<form action="index.php" method="post" name="adminForm" id="adminForm">
    <fieldset class="filter">
        ...
        <tr>
            <td>
                <label for="search"><?php echo JText::_( 'ADMIN_REGION_LABEL_SEARCH' ); ?>:</label>
                <input type="text" name="search" id="search"
                value="<?php echo $this->state->search; ?>" size="60"
                title="<?php echo $this->escape(JText::_( 'ADMIN_REGION_TOOLTIP_SEARCH' )); ?>" />
                <button type="submit"><?php echo JText::_( 'ADMIN_REGION_LABEL_GO' ); ?></button>
                <button type="button"
                    onclick="document.getElementById('search').value='';this.form.submit();">
                    <?php echo JText::_( 'ADMIN_REGION_LABEL_CLEAR' ); ?>
                </button>
            </td>
            <td>
                <label for="level">
                    <?php echo JText::_( 'ADMIN_REGION_LABEL_LEVELS' ); ?>
                </label>
	...
	<table class="adminlist">
		<thead>
			<tr>
				<th width="20"></th>
				<th width="30">
					<?php echo JText::_( 'ADMIN_REGION_LABEL_ID'); ?>
				</th>
				<th class="left">
					<?php echo JText::_( 'ADMIN_REGION_LABEL_REGION'); ?>
				</th>
				<th class="left">
					<?php echo JText::_( 'ADMIN_REGION_LABEL_SITES'); ?>
				</th>
			</tr>
		</thead>
		<tfoot>
...
if (count($this->items)>0) {
    ...
    foreach ($this->items as $item) { ?>
        ...
        <?php
        $link = JRoute::_("index.php?option=com_hub2&view=region&model=region&task=edit&cid[]=".$item->id );
