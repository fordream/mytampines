...
<table>
	<tr>
		<td><label for="search"><?php echo JText::_( 'ADMIN_TAG_LABEL_SEARCH' ); ?>:</label> <input
			type="text" name="search" id="search"
			value="<?php echo $this->state->search; ?>" size="60"
			title="<?php echo $this->escape(JText::_( 'ADMIN_TAG_TOOLTIP_SEARCH' )); ?>" />
		<button type="submit"><?php echo JText::_( 'ADMIN_TAG_LABEL_GO' ); ?></button>
		<button type="button"
			onclick="document.getElementById('search').value='';this.form.submit();">
			<?php echo JText::_( 'ADMIN_TAG_LABEL_CLEAR' ); ?></button>
		</td>
		<td>&nbsp;</td>
	</tr>
</table>
...
<table class="adminlist">
	<thead>
		<tr>
			...
			<?php echo JHTML::_( 'grid.sort', 'ADMIN_TAG_LABEL_ID', 's.id',
			             $this->state->orderDirn, $this->state->orderCol ); ?>
			</th>
			<th>
			<?php echo JHTML::_( 'grid.sort', 'ADMIN_TAG_LABEL_NAME', 's.name',
			             $this->state->orderDirn, $this->state->orderCol ); ?>
			</th>
			<th>
			<?php echo JHTML::_( 'grid.sort', 'ADMIN_TAG_LABEL_ALIAS', 's.alias',
			             $this->state->orderDirn, $this->state->orderCol ); ?>
			</th>
		</tr>
	</thead>
...
if (count($this->items)>0) {
    foreach ($this->items as $item) { ?>
        ...
        <?php
        $link = JRoute::_( 'index.php?option=com_hub2&view=tag&model=tag&task=edit&cid[]='.$item->id );
        ...
        ?>
    			...
    			<td align="left">
        <?php if ($canEdit) { ?>
                    <a href="<?php echo $link;?>"><?php echo $item->name; ?>
    			</a>
            <?php
        } else {
            echo $item->name;
        ...
        ?>
    			...
    			<td align="left">
        <?php if ($canEdit) { ?>
                    <a href="<?php echo $link;?>"><?php echo $item->alias; ?>
    			</a>
            <?php
        } else {
            echo $item->alias;
...
<input type="hidden" name="option" value="com_hub2" /> <input
	...
	value="tag" /> <input type="hidden" name="model" value="tag" /> <input
