...
class WSDLStruct {
    ...
    /** @var domElement[] references to the binding operation tags.
     * servicename as first key, operationname as second */
    ...
    public function __construct($tns, $url, $type = SOAP_RPC, $use = SOAP_ENCODED) {
        if($type != SOAP_RPC     && $type != SOAP_DOCUMENT) {
            throw new Exception("Webservice type parameter should be either
SOAP_RPC or SOAP_DOCUMENT");
        }
        if($use  != SOAP_ENCODED && $use  != SOAP_LITERAL) {
            throw new Exception("Webservice use parameter should be either
SOAP_ENCODED or SOAP_LITERAL");
            ...
            foreach((array)$service->methods as $operation){
                ...
                if(count($parameters)>0 || self::CREATE_EMPTY_INPUTS){
                    ...
                    foreach((array)$parameters as $parameterName => $parameter){
                        /** Hyperlocalizer modification - PHP SOAP wants the
                         * Request message parts to be unique for each function
                         * if we have multiple functions with the same variable names
                         * then the original method gives the same parts and
                         * then SOAP server does not respond! so make the parts
                         * unique
                         */
                        $para[$parameterName.$messageName] = $parameter->type;
                        /** end Hyperlocalizer */
                    }
                ...
                if(strtolower(trim($operation->return))!='void'){
                    ...
                    Array($operation->name."Return" => $operation->return));
                }
            }
    ...
    private function addOperation($operationName, $serviceName) {
        ...
        $soapOperationTag->setAttribute("style",
        ($this->binding_style == SOAP_RPC)? "rpc" : "document");
    ...
    private function addBinding($serviceName) {
        ...
        $soapBindingTag->setAttribute("style",
        ($this->binding_style == SOAP_RPC)? "rpc" : "document");
    ...
    private function addMessage($name, $parts) {
        ...
        foreach((array)$parts as $partName => $partType){
            ...
            if ($t = IPXMLSchema::checkSchemaType(strtolower($partType))) {
                ...
                $partName = (substr($partType,-2) == "[]")?
                substr($partType,0,strpos($partType,"["))."Array":$partType;
