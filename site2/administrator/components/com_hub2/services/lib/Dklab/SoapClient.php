...
class Dklab_SoapClient extends SoapClient {
     ...
     * @see SoapClient
     */
    public function __construct($wsdl, $options = array(), $rand) {
        parent::__construct($wsdl, $options);
        // this can be an error need to check
        if (!is_soap_fault($this)) {
            $this->_clientOptions = is_array($options)? array() + $options : array();
            if ($rand !== self::$_rand) {
                Dklab_SoapClient_Request::clean();
            }
            self::$_rand = $rand;
        }
    }
     ...
     */
    public function __soapCall($functionName, $arguments, $options = array(),
    $inputHeaders = null, &$outputHeaders = null) {
        ...
        $request = new Dklab_SoapClient_Request($this, $this->_recordedRequest, $args,
        $this->_clientOptions);
    ...
    public function __soapCallForced($forcedResponse, $origArgs) {
        ...
        try {
            ...
            // it does not support "parent::" construction. And we cannot
            // call is "statically" because of E_STRICT.
            $result = parent::__soapCall($origArgs[0], $origArgs[1],
            isset($origArgs[2])? $origArgs[2] : array(), @$origArgs[3], $origArgs[4]);
    ...
    public function __get($key) {
        if ($key == "async") {
            ...
            throw new Exception("Attempt to access undefined property " .
            get_class($this) . "::$key");
        }
    }
}
...
class Dklab_SoapClient_Request
{
     ...
     */
    public function __construct(Dklab_SoapClient $client, $request,
    $callArgs, $clientOptions) {
        if (!self::$_curl) {
...
class Dklab_SoapClient_Curl
{
    ...
    public function addRequest($curlOptions) {
        ...
        if (isset($curlOptions['response_validator']) &&
        is_callable($curlOptions['response_validator'])) {
    ...
    private function _storeResponses(&$nRunning = null) {
        while ($done = curl_multi_info_read($this->_handler)) {
            ...
            $response['result_timeout'] = $response["result"] ===
            CURLE_OPERATION_TIMEOUTED? ($response["request_size"] <= 0? 'connect' : 'data') : null;
    ...
    private function _addCurlRequest(stdClass $request, $key) {
        ...
        $min = min(
        isset($request->options[CURLOPT_TIMEOUT_MS])? $request->options[CURLOPT_TIMEOUT_MS] : 10000,
        isset($request->options[CURLOPT_CONNECTTIMEOUT_MS])?
        $request->options[CURLOPT_CONNECTTIMEOUT_MS] : 10000
        );
    ...
    private function _getCurlNextTimeoutDelay() {
        $time = microtime(true);
        $min = 10000; /** Hyperlocalizer in MS **/
        ...
        $min = max($min/1000, 0.001); // HYPERLOCALIZER required in seconds
        return $min;
    }
    ...
    private function _execCurl($waitForAction) {
        ...
        if ($waitForAction) {
            /** Hyperlocalizer - somehow the timeout does not work **/
            // $d = $this->_getCurlNextTimeoutDelay();
            curl_multi_select($this->_handler);
        }
        while (curl_multi_exec($this->_handler, $nRunningCurrent) == CURLM_CALL_MULTI_PERFORM) {

        }
        /** Hyperlocalizer - store responses only if wait for response */
        if ($waitForAction) {
            // Store appeared responses if present.
            $this->_storeResponses($nRunningCurrent);
            if ($nRunningCurrent == 0) {
                curl_multi_close($this->_handler); // close all if none running
            }
        }
        /** end Hyperlocalizer */
