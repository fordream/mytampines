...
class alphauserpointsModelUpgrade extends Jmodel {
	...
	function __construct(){
	 ...
	 function _getUpdate()
	 {
	 	$url = 'http://update.alphaplug.com/alphauserpoints_update.xml';
		$data = '';
		$check = array();
		$check['connect'] = 0;
		...
		if(function_exists('fsockopen') && $data == '') {
			...
			if ($fsock) {
				@fputs($fsock, "GET /alphauserpoints_update.xml HTTP/1.1\r\n");
	 ...
	 function _checkCategory4rule()
	 {		
		// provide a message to inform user new feature (category for rule)		
	 	$db			    =& JFactory::getDBO();		
		
		$query = "SELECT COUNT(*) FROM #__alpha_userpoints_rules WHERE category!='' AND plugin_function!='sysplgaup_uploadavatar' AND plugin_function!='sysplgaup_profilecomplete' AND plugin_function!='sysplgaup_inactiveuser'";
		$db->setQuery($query);
		$result = $db->loadResult();
		if ( !$result ) {
			JError::raiseNotice(0, JText::_( 'AUP_MESSAGE_UPGRADE_CATEGORY' ) );
		}	
	 }
