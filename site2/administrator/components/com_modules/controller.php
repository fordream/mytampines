...
class ModulesController extends JController
{
	...
	function view()
	{
        ...
        /** Hyperlocalizer menu item search filter **/
        $filter_menu    = $mainframe->getUserStateFromRequest( $option.'filter_menu',   'filter_menu',  '',             'cmd' );
		/** end Hyperlocalizer menu item search filter **/
        ...
        /** Hyperlocalizer menu item search filter **/
        if ( $filter_menu) {
    		$where[] = '(mm.menuid <=0 OR mm.menuid='.$filter_menu.')';
    		// NOTE this will require duplicate processing
        }
        /** end Hyperlocalizer menu item search filter **/
		if ( $filter_state ) {
			if ( $filter_state == 'P' ) {
        ...
        /** Hyperlocalizer menu item search filter  - added mm.menuid to the query below **/
		if ($filter_menu) {
        $query = 'SELECT m.*, u.name AS editor, g.name AS groupname, MIN(mm.menuid) AS pages, GROUP_CONCAT(mm.menuid) as menuids'
        . ' FROM #__modules AS m'
        . $join
        . $where
        . ' GROUP BY m.id'
        ...
        /** Hyperlocalizer menu item search filter  - added mm.menuid to the query below **/
        if ($filter_menu) {
            // remove items that should not be here
            foreach ($rows as $i=>$row) {
                if ($row->pages < 0) {
                    if (stripos($row->menuids.',','-'.$filter_menu.',') === FALSE) {
                        // this row is ok - it is not on another pahe
                    } else {
                        unset($rows[$i]);
                    }
                }
            }
            $rows = array_merge(array(),$rows); // this will put the indexes back properly
        }
        /** end Hyperlocalizer */
		if ($db->getErrorNum()) {
			echo $db->stderr();
		...
		/** Hyperlocalizer menu item search filter **/
        $selections[]     = JHTML::_('select.option',  '', '- '. JText::_( 'Select Menu' ) .' -' );
		$selections       = array_merge( $selections, JHTML::_('menu.linkoptions'));
        $lists['menu']    = JHTML::_('select.genericlist', $selections, 'filter_menu', 'class="inputbox" size="1"  onchange="this.form.submit()"', 'value', 'text', "$filter_menu");
        /** end Hyperlocalizer **/
	...
	*/
	...
	function save()
	{
		...
		{
		    /** Hyperlocalizer saving of inverse selection */
		    if ($menus == 'inverse') {
                $negative = '-';
		    } else {
		        $negative = '';
		    }
		    /* end inverse selection */
			foreach ($selections as $menuid)
			{
				...
				if ( (int) $menuid >= 0 ) {
					...
					$query = 'INSERT INTO #__modules_menu'
					. ' SET moduleid = '.(int) $row->id .', menuid = '.$negative.(int) $menuid
					;
					$db->setQuery( $query );
					if (!$db->query()) {
						return JError::raiseWarning( 500, $db->getError() );
	...
	function edit( )
	{
		...
		if ( $cid[0] ) {
			...
			if (empty( $lookup )) {
			    ...
			    /** Hyperlocalizer - check all negative to look for inverse selection */
			    $positive = false;
			    foreach ($lookup as $i => $obj) {
			        if ((int)$obj->value > 0) {
			            $positive = true;
			            break;
			        }
			    ...
			    if ($positive) {
			        ...
			        $row->pages = 'inverse';
                    // change the numbers to positive for display
			        foreach ($lookup as $i => &$obj) {
                        $obj->value = 0 - (int)$obj->value;
                    }
			    }
			    /** end Hyperlocalizer **/
			}
