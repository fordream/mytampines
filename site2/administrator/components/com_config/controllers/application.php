...
class ConfigControllerApplication extends ConfigController
{
	...
	function save()
	{
		...
		/** Hyperlocalizer modification - save the master config to another file **/
		if (IS_MANAGER) {
            // Get the path of the configuration file
            $fname2 = JPATH_CONFIGURATION.DS.'manager_configuration.php';
            if (JFile::write($fname2, $config->toString('PHP', 'config', array('class' => 'ManagerConfig')))) {
                $msg = JText::_('The Configuration Details have been updated');
            } else {
                $msg = JText::_('ERRORCONFIGFILE');
            }
