...
?>
<form action="<?php echo JRoute::_('index.php?option=com_control&view=test3d&model=test');?>" method="post" name="adminForm">
	<fieldset class="filter clearfix">
		<ol class="left">
			<li>
				<label for="search"><?php echo JText::_('JX Search Users'); ?>:</label>
				<input type="text" name="search" id="search" value="<?php echo $this->state->search; ?>" size="60" title="<?php echo JText::_('JX Search Users Desc'); ?>" />
			</li>
			<li>
				<label for="section_value">
					<?php echo JText::_('JX Filter'); ?>:
				</label>
				<select name="section_value" id="section_value" class="inputbox">
			...
			<li>
				<select name="axo_section_value" id="axo_section_value" class="inputbox">
					<option value=""><?php echo JText::_('- Select AXO Section -');?></option>
				<?php
					echo JHTML::_('select.options', $this->axoSectionValues, 'value', 'text', $this->state->axo_section_value);
				?>
				</select>
			</li>
			<li>
	...
	<table class="adminlist">
		...
		<?php
			...
			<tr class="">
				...
				<td>
					<?php if ($this->state->section_value == '') : ?>
					<?php echo $item->axo_section_name; ?><br />
					<?php endif; ?>
					<?php echo $item->axo_name; ?>
				</td>
				<td>
				...
				<td>
					<?php
						$href	= 'index.php?option=com_control&view=debug' .
								...
								'&axo_section_value='.$item->axo_section_value.'&axo_value='.$item->axo_value;
					?>
