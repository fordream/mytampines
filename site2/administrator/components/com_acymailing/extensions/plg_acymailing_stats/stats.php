...
class plgAcymailingStats extends JPlugin
{
	...
	function acymailing_replaceusertags(&$email,&$user){
		...
		$width = empty($widthsize) ? '' : ' width="'.$widthsize.'" ';
		$height = empty($heightsize) ? '' : ' height="'.$heightsize.'" ';
		$statPicture = '<img alt="'.$this->params->get('alttext','').'" src="'.acymailing_frontendLink('index.php?option=com_acymailing&ctrl=stats&mailid='.$email->mailid.'&subid='.$user->subid).'"  border="0" '.$height.$width.'/>';
