...
class plgAcymailingTagsubscriber extends JPlugin
{
	...
	function replaceSubTag(&$allresults,$i,&$user){
		...
		if(!empty($mytag->type)){
			if($mytag->type == 'date'){
				...
				$replaceme = acymailing_getDate($replaceme);
			}
		}
	...
	function onAcyProcessFilterCount_acymailingfield(&$query,$filter,$num){
		if($filter['map'] == 'created' AND !is_numeric($filter['value'])){
	 	...
	 	$myquery = 'SELECT COUNT(sub.subid) FROM #__acymailing_subscriber as sub WHERE '. $query->convertQuery('sub',$filter['map'],$filter['operator'],$filter['value']);
	 	$db =& JFactory::getDBO();
	 	$db->setQuery($myquery);
