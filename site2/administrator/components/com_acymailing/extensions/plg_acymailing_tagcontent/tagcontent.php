	...
	<?php
	 ...
	 function _replaceContent(&$results,$i){
		...
		if(!empty($tag->created)){
			if($tag->type == 'title') $result .= '<br/>';
			$result .= '<span class="createddate">'.JHTML::_( 'date', $article->created, JText::_('DATE_FORMAT_LC2')).'</span><br/>';
		}
		...
		foreach($results as $var => $allresults){
			foreach($allresults[0] as $i => $oneTag){
				...
				if(!empty($parameter->order)){
					...
					$query .= ' ORDER BY a.`'.acymailing_secureField($ordering[0]).'` '.acymailing_secureField($ordering[1]);
				}
				$start = '';
